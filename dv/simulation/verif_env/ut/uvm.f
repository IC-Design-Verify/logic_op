// =============================================================================
// UVM/TB filelist  --  UVM library + testbench for logic_op
// -----------------------------------------------------------------------------
// Uses the official Accellera UVM (IEEE 1800.2-2020) from uvm-verilator/
// (chipsalliance/uvm-verilator), compiled in by Verilator.
//
// NOTE on separate compilation:
//   The full UVM simulation MUST be Verilated together with the DUT, because
//   logic_op_tb_top instantiates the DUT in SystemVerilog and drives its ports
//   via hierarchical assigns. So build/run use  -f dut.f -f uvm.f  together.
//   (True DUT-library + separate-TB linking only works for a C++ testbench.)
//
// Requires env var: PROJ_HOME = logic_op repo root.
// The VCS `logic_op_test` program block is intentionally NOT listed; run_test()
// is launched from logic_op_tb_top under the -DVL_RUN_TEST guard.
...
//////////////////////////////
// RTL helper models (TB-side, compiled but not instantiated)
//////////////////////////////

...
// apb: apb
assign `MERGE_ITF_NAME(`LOGIC_OP_ITF_PRE_NAME, apb_vif).PCLK    = `MERGE_ITF_NAME(`LOGIC_OP_ITF_PRE_NAME, sys_clk.CLOCK);
assign `MERGE_ITF_NAME(`LOGIC_OP_ITF_PRE_NAME, apb_vif).PRESETn = `MERGE_ITF_NAME(`LOGIC_OP_ITF_PRE_NAME, sys_rst.RESET);
assign `LOGIC_OP_PATH.psel             = `MERGE_ITF_NAME(`LOGIC_OP_ITF_PRE_NAME, apb_vif).PSEL[0]  ;
assign `LOGIC_OP_PATH.penable          = `MERGE_ITF_NAME(`LOGIC_OP_ITF_PRE_NAME, apb_vif).PENABLE  ;
assign `LOGIC_OP_PATH.paddr            = `MERGE_ITF_NAME(`LOGIC_OP_ITF_PRE_NAME, apb_vif).PADDR    ;
assign `LOGIC_OP_PATH.pwrite           = `MERGE_ITF_NAME(`LOGIC_OP_ITF_PRE_NAME, apb_vif).PWRITE   ;
assign `LOGIC_OP_PATH.pwdata           = `MERGE_ITF_NAME(`LOGIC_OP_ITF_PRE_NAME, apb_vif).PWDATA   ;
assign `MERGE_ITF_NAME(`LOGIC_OP_ITF_PRE_NAME, apb_vif).PRDATA  = `LOGIC_OP_PATH.prdata;
assign `MERGE_ITF_NAME(`LOGIC_OP_ITF_PRE_NAME, apb_vif).PREADY  = 'b1                     ;
